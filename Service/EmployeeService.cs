...
{
    public async Task<(IEnumerable<EmployeeDto> employeeDtos, MetaData metaData)> GetEmployeesAsync(
        Guid companyId,
    ...
    )
    {
        ...
        var employees = await repository.Employee.GetEmployeesAsync(
            companyId,
            employeeParameters,
            trackChanges
        );
        ...
        logger.LogInfo(
            $"All employees for company with id: {companyId} were returned successfully."
        );
    ...
    public async Task<IEnumerable<EmployeeDto>> GetByIdsAsync(
        Guid companyId,
    ...
    )
    {
        ...
        var employees = await repository.Employee.GetByIdsAsync(
            companyId,
            employeeIds,
            trackChanges
        );
    ...
    private async Task CheckIfCompanyExists(Guid companyId, bool trackChanges) =>
        _ =
            await repository.Company.GetCompanyAsync(companyId, trackChanges)
