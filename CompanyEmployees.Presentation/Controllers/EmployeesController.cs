...
public class EmployeesController(IServiceManager service) : ControllerBase
{
    ...
    {
        var (employeeDtos, metaData) = await service.EmployeeService.GetEmployeesAsync(companyId, employeeParameters, trackChanges: false);
    ...
    {
        var employee = await service.EmployeeService.CreateEmployeeForCompanyAsync(companyId, employeeForCreationDto, trackChanges: false);
    ...
    {
        var companies = await service.EmployeeService.GetByIdsAsync(companyId, employeeIds, trackChanges: false);
        return Ok(companies);
    }

    [HttpPost("collection")]
    public async Task<IActionResult> CreateEmployeeCollection(Guid companyId, [FromBody] IEnumerable<EmployeeForCreationDto> employeeForCreationDtos)
    {
        var (employeeDtos, employeeIds) = await service.EmployeeService.CreateEmployeesForCompanyCollectionAsync(companyId, employeeForCreationDtos);
    ...
    { 
        await service.EmployeeService.DeleteEmployeeForCompanyAsync(companyId, employeeId, trackChanges: false); 
        return NoContent(); 
    }
    ...
    {
        await service.EmployeeService.UpdateEmployeeForCompanyAsync(companyId, employeeId, employeeForUpdateDto, companyTrackChanges: false, employeeTrackChanges: true);
        return NoContent(); 
    }
    ...
    { 
        ...
        (EmployeeForUpdateDto employeeForUpdateDto, Employee employee) = await service.EmployeeService.GetEmployeeForPatchAsync(companyId, employeeId, companyTrackChanges: false, employeeTrackChanges: true);
