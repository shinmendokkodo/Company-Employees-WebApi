...
public class CompaniesController(IServiceManager service) : ControllerBase
{
    ...
    {
        var (companyDtos, companyIds) = await service.CompanyService.CreateCompanyCollectionAsync(companyForCreationDtos);
    ...
    { 
        await service.CompanyService.UpdateCompanyAsync(companyId, companyForUpdateDto, trackChanges: true); 
        return NoContent(); 
    }

    [HttpPatch("{companyId:guid}")]
    public async Task<IActionResult> PartiallyUpdateCompany(Guid companyId, [FromBody] JsonPatchDocument<CompanyForManipulationDto> jsonPatchDocument)
    {
        ...
        var (companyForManipulationDto, company) = await service.CompanyService.GetCompanyForPatchAsync(companyId, trackChanges: true);
